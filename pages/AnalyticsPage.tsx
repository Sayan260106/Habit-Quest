...
const AnalyticsPage: React.FC<AnalyticsPageProps> = ({ user }) => {
  ...
  const runDeepAnalysis = async () => {
    ...
    try {
      ...
      Identify major patterns, streak break triggers, productive time, risky days, reasons for misses, and 5 improvements.`;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              streakBreaks: { type: Type.STRING },
              productiveTime: { type: Type.STRING },
              riskyDays: { type: Type.STRING },
              likelyReasons: { type: Type.ARRAY, items: { type: Type.STRING } },
              suggestions: { type: Type.ARRAY, items: { type: Type.STRING } }
            },
            required: ["patterns", "streakBreaks", "productiveTime", "riskyDays", "likelyReasons", "suggestions"]
          }
        }
      });
  ...
  const generateWeeklyReport = async () => {
    ...
    try {
      ...
      const prompt = `Generate a highly detailed weekly progress report based on this 7-day data: ${JSON.stringify(weeklyLogs)}. Habits: ${habitNames}.`;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              trendChanges: { type: Type.STRING },
              consistencyPercentage: { type: Type.NUMBER },
              motivationAnalysis: { type: Type.STRING },
              summary: { type: Type.STRING }
            },
            required: ["strongestHabits", "weakestHabits", "trendChanges", "consistencyPercentage", "motivationAnalysis", "summary"]
          }
        }
      });
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
        ...
        <div className="flex flex-wrap gap-4">
          <button
            ...
            disabled={reporting}
            className={`px-6 py-3 text-[10px] font-black rounded-2xl uppercase tracking-widest transition-all active:scale-95 ${reporting
          ...
          <div className="flex bg-slate-900 p-1 rounded-2xl border border-slate-800 shadow-xl">
            {(['daily', 'monthly'] as TimeRange[]).map((range) => (
              <button
                ...
                className={`px-5 py-2 text-[10px] font-black rounded-xl uppercase tracking-widest transition-all ${timeRange === range ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-600 hover:text-slate-400'
                  }`}
              >
                {range}
              </button>
        ...
        <div className="bg-slate-950 rounded-[38px] p-8 md:p-12 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
            ...
            <button
              ...
              disabled={analyzing}
              className={`whitespace-nowrap px-8 py-4 font-black rounded-2xl flex items-center gap-3 transition-all active:scale-95 text-[10px] uppercase tracking-widest ${analyzing
