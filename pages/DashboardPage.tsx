...
const DashboardPage: React.FC<DashboardPageProps> = ({ user }) => {
  ...
  const confirmDelete = () => {
    if (habitToDelete) {
      ...
      setLogs(prev => prev.filter((l) => l.habitId !== habitToDelete.id));
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 bg-slate-950 min-h-screen text-slate-100">
        ...
        <div className="flex-grow w-full space-y-4">
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-xs font-mono text-emerald-500/80 uppercase tracking-widest">XP {currentXP} / TARGET 100</p>
            </div>
                    ...
                    <tr key={habit.id} className="hover:bg-slate-800/20 transition-all duration-200 group/row">
                      <td className="sticky left-0 z-10 bg-slate-900 group-hover/row:bg-slate-800/90 px-8 py-6 whitespace-nowrap border-r border-slate-800/50 transition-all">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => setHabitToDelete(habit)}
                            className="p-2 opacity-0 group-hover/row:opacity-100 text-slate-600 hover:text-red-500 transition-all scale-75 hover:scale-100 z-10 relative"
                            title="Purge Node">
      ...
      {habitToDelete && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="bg-slate-900 w-full max-w-md rounded-[32px] sm:rounded-[40px] border border-red-500/30 shadow-[0_0_50px_rgba(239,68,68,0.2)] relative z-10 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-1">
              <div className="bg-slate-950 rounded-[30px] sm:rounded-[38px] p-8 sm:p-10 text-center">
                <div className="w-16 h-16 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-red-500/20">
                  <i className="fas fa-trash-can text-red-500 text-2xl animate-bounce"></i>
                </div>
                <h3 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter mb-2">Execute Purge?</h3>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Irreversible Data Loss Warning</p>

                <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 mb-8 flex items-center gap-4 text-left">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white" style={{ backgroundColor: habitToDelete.color }}>
                    <i className={`fas ${habitToDelete.icon}`}></i>
                  </div>
                  <div>
                    <p className="text-xs font-black text-slate-200 uppercase tracking-tight">{habitToDelete.name}</p>
                    <p className="text-[9px] font-mono text-slate-500 uppercase">Protocol ID: {habitToDelete.id}</p>
                  </div>
                </div>

                <p className="text-xs text-slate-400 font-medium leading-relaxed mb-8">
                  Confirming this protocol will permanently erase all historical data nodes and synchronization logs for this habit.
                </p>

                <div className="flex flex-col gap-3">
                  <button
                    onClick={confirmDelete}
                    className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-black rounded-xl sm:rounded-2xl shadow-xl shadow-red-600/20 transition-all active:scale-95 text-xs uppercase tracking-[0.3em]"
                  >
                    Execute Purge
                  </button>
                  <button
                    onClick={() => setHabitToDelete(null)}
                    className="w-full py-4 bg-slate-900 text-slate-500 font-black rounded-xl sm:rounded-2xl hover:bg-slate-800 transition-colors uppercase text-[10px] tracking-widest"
                  >
                    Abort Protocol
                  </button>
                </div>
